...
以上 8 套是给屏幕挑的，展板（`nextcourse themes`）里也只有这 8 套。另有一套**只在导出时出场**：

| 方案名 | 适合场景 |
|---|---|
| `print-light` | 纯白 + 深灰、无阴影无满版色块 — **仅供 `nextcourse pdf --theme print-light`**，给学员打印到纸上用 |

`print-light` 不进展板（头注释里写了 `Gallery: off`），也**不要**写进 `course.meta.md` 的 `theme:`
——那会让你在投影上讲一份白底课件。它派生自 `warm-sand`，只按"纸"改了四组值：底色压纯白、
阴影全撤（卡片靠 `--border-card` 的 1px 发丝线分隔）、模块封面留白只剩标题、代码块翻成浅底深字。
...
- **别给「底色等于页底色」的元素加阴影。** 页码胶囊踩过：它的 `background` 取 `--bg-slide`，
  在 dark-ocean 里与页底色完全同色，于是屏幕上看不出问题，一进 PDF 就只剩那圈
  `--shadow-card`——而 Chrome 会把大模糊阴影位图化、露出外框，成品每页底部浮着一个深色方块。
  现在导出时的页码不再做胶囊，只是一个数字（见 `themes/standard.css` 末尾）。
