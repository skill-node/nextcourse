...
```
package/
├── facilitator-guide.md   Facilitator guide (teaching points, activity scripts, slide notes)
├── workbook.md            Learner workbook (tasks, write-in areas, self-check lists)
├── assessment.md          Evaluation plan (L1 survey + L2 assessment)
├── rubric.md              Scoring rubric (dimension x level, every cell an observable behaviour)
├── facilitation.md        Facilitation design (timeline, grouping, points rules)
├── content-dev.md         Content development (SME interviews, case library, schedule)
├── action-plan.md         Learner 30-day action commitment
├── alignment.md           Alignment matrix (generated by check)
└── html/                  What the client receives: opens on a double-click, prints without losing styling
```
...
what this project is really about. `package/alignment.md` is worth a separate look:
