...
课件是 HTML，但课后发到学员手里的那一份，通常还是 PDF。一条命令，
一页一张幻灯片，配色、字体、版式与屏幕上完全一致：

```bash
nextcourse pdf <课程名>                       # → courses/<课程名>/pdf/deck.pdf
nextcourse pdf <课程名> --theme print-light   # 学员要打印到纸上的浅色版
```

**别在浏览器里 Cmd+P。** reveal.js 自带一套「印在 A4 纸上」的打印样式：背景刷白、
正文 20pt 纯黑、装饰全隐藏。所有「打印出来跟课件完全不是一回事」都是它干的，
不是配色坏了。`nextcourse pdf` 绕开的正是这套样式 —— DOM 一个节点都不挪，只用 CSS 分页，
所以设计系统里那些 `.slides > section` 规则照常命中，纸上和屏幕上长得一样。

发出去的那一份通常还要两件事：

- **`--theme print-light`** —— 一套按「纸」做的配色：底色压纯白、阴影全撤换成 1px 发丝线、
  模块封面留白只剩标题、代码块翻成浅底深字。版式与字号一个像素不动，因为配色本来就是
  独立的一层。它不要写进 `course.meta.md` 的 `theme:` —— 写了就等着在投影上讲一份白底课件。
- **`<section>` 上的 `data-print="off"`** —— 现场演示页、客户敏感案例，凡是不该外发的页，
  导出时整页摘掉。页码是 CSS 计数器，会自己连号；授课用的 deck 照常带着这一页。
  真相只有一份，在 `slides/` 里。
